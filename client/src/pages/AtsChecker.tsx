...
const AtsChecker: React.FC = () => {
  ...
  const analyzeResume = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/ats/check', formData, {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <div className="text-center mb-12">
        ...
        <p className="text-gray-600">See how your resume performs against modern hiring systems.</p>
      </div>
      ...
      <AnimatePresence mode="wait">
        {!report ? (
          ...
          >
            {!file ? (
              <div {...getRootProps()} className="w-full text-center cursor-pointer group">
                <input {...getInputProps()} />
              ...
              <div className="relative w-48 h-48 mx-auto mb-6">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <motion.circle
                    ...
                    animate={{ strokeDashoffset: 552 - (552 * report.score) / 100 }}
                    transition={{ duration: 1.5, ease: "easeOut" }}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-5xl font-extrabold text-gray-900">{report.score}</span>
                  <span className="text-gray-500 font-medium">ATS Score</span>
                </div>
              </div>
              <button onClick={reset} className="text-primary-600 font-medium hover:underline">
              ...
              <div className="card">
                ...
                <div className="flex flex-wrap gap-2">
                  {report.missingKeywords.map((kw: string) => (
                    <span key={kw} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm">
                      {kw}
                    </span>
                  ))}
                </div>
